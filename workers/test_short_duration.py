...
logging.config.fileConfig('testapi_logging.conf')
...
# a month in microseconds
    ...
    start_time = now_ms - duration
    end_time = start_time + duration
    full_zoon_event_api_request_url = '%s/%s' % (zoon_event_api_request_url, project)
    ...
    while (start_time <= now_ms):
        ...
        while (num_results == per_page):
            ...
            logger.info("requesting %s from %s to %s" % (full_zoon_event_api_request_url, datetime.datetime.fromtimestamp(start_time/1000).strftime(dateformat), datetime.datetime.fromtimestamp(end_time/1000).strftime(dateformat)))
            json_obj = get_json_from_api(full_zoon_event_api_request_url, payload, headers)
            if json_obj is None:
                num_results = 0
            ...
            page = page + 1
            logger.info("request yielded %d classifications" % total_results)
