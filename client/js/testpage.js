...
var activeSceneId = 'none';

var config = {};

function loadCallback(err, progress) {
  if (err) {
    window.alert('Soundengine failed to load:'+ err.message);
    $('.progress').text('Sound engine failed to load properly');
    return;
  }
  debug('progress', progress);
  $('.progress').text('loading files...'+progress+'%');
...
function triggerB(){
   var filename = ZN.soundengine.triggerSampler(1);
   debug('triggered', filename);
}
...
function displayInfo(info){
  $('div.scene-info').text(JSON.stringify(info, null, 4));
}

function displaySceneId(id){
  $('.scene_id').text(id);
}
...
$(document).ready(function() {
  ...
  $.getJSON('/sound/config', function(data) {
    debug('config', data);
    ...
    // ZN.soundengine.init(config.scenes, config.options, loadCallback);
    ZN.soundengine.init(config, loadCallback);
  }); // getJSON

  document.onkeydown = function(evt) {
    evt = evt || window.event;
    debug('keydown', evt.keyCode);

    switch (evt.keyCode) {
      case 83: // s
        startstop();
        break;
      case 80: // p next scene
        nextScene();
        break;
      case 79: // o prev scene
        previousScene();
        break;
  ...
  $('button.trig_a').click(function() {
    triggerA();
  });
  $('button.trig_b').click(function() {
    triggerB();
  });
